...
import com.app.security.CustomInstructorDetails;
...
public class JwtUtils {
	...
	public String generateJwtToken(Authentication authentication) {
		...
		CustomInstructorDetails instructorPrincipal = (CustomInstructorDetails) authentication.getPrincipal();
//JWT : userName,issued at ,exp date,digital signature(does not typically contain password , can contain authorities
		return Jwts.builder() // JWTs : a Factory class , used to create JWT tokens
				.setSubject((instructorPrincipal.getUsername())) // setting subject part of the token(typically user
															// name/email)
				...
				.claim("authorities", getAuthoritiesInString(instructorPrincipal.getAuthorities()))
